...
const menuItem_styles =
  "py-[0.675rem] px-4 w-40 align-center gap-3 text-primary-200 rounded-[0.75rem] hover:bg-primary-700"

export default function ListCard({ list }) {
  const { modalDispatch } = useAppContext()
  const { userState } = useUserContext()
  const isOwner = userState.id === list.creator_id
  const { data: bookmarks, isLoading } = useLoader(() =>
  ...
  if (!isLoading) {
    a = bookmarks
      ?.slice(0, thumbsCount)
      ?.map((bookm) => ({ id: bookm.id, media: bookm.media }))
            ...
            <Dropdown.Container strategy="portal">
              ...
              >
                <Dropdown.MenuItem className={menuItem_styles} onClick={onEdit}>
                  <span>Edit</span>
                  <Icon className="ml-auto" size="md" svg={<PencilIcon />} />
                </Dropdown.MenuItem>
                <Dropdown.MenuItem
                  className={`${menuItem_styles} text-red-600`}
                  onClick={onDelete}
                >
                  <span>Delete</span>
                  <Icon className="ml-auto" size="md" svg={<TrashIcon />} />
                </Dropdown.MenuItem>
              </Dropdown.Menu>
            </Dropdown.Container>
          )}
...
function Thumb({ thumb, fallback = false, className, ...rest }) {
  ...
  return (
    ...
    >
      {thumb && (
        <img
          className="w-16 aspect-[0.67]"
          src={`${IMAGES_URL}w500${mediaDetails.poster_path}`}
        />
      )}
      {fallback && (
